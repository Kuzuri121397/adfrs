			...
			if (isset($_POST['submit'])) 
			{
				...
				$incoming_remarks=$_POST['incoming_remarks'];
					...
					if (!empty($fileTmpName)) 
					{
							...
							if($numFile > 0)
							{
								echo"<script>alert('A file already exists!'); window.location.href='../pages/incoming.php?id=".$user_id."'</script>";
							}
							else
							{
								$folder="../system/files";
									...
									if ($fileError === 0) 
									{
										if($filesize < 1000000)
										{
            								...
            								while ($rowResult=mysqli_fetch_array($selectResult)) 
            								{
            									...
            									echo"<script>alert('Updated Successfully!'); window.location.href='../pages/incoming.php?id=".$user_id."'</script>";
            								}
