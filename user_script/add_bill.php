					...
					if(!empty($fileTmpName))
						{
							...
							$sql2="INSERT INTO t_bill_info(payee, bill_month, bill_year,date_receive, bill_amount, due_date, receipt_no,status_id)VALUES('".$payee."', '".$bill_month."', '".$bill_year."', '".$date_receive."', '".$bill_amount."', '".$due_date."', '".$receipt_no."', '".$status."')";
            						...
            						if(in_array($fileActualExt,$allowed))
            						{
            							if($fileError === 0)
            							{
            								if($filesize < 1000000)
                							{
            									...
            									echo"<script>alert('Record Saved!'); window.location.href='../user/bill.php?id=".$user_id."'</script>";
											}
											else
											{
						...
						{
							echo"<script>alert('Please Upload a file!'); window.location.href='../user/bill.php?id=".$user_id."'</script>";
						}
			}
			else
			{
				echo "<script>alert('Adding Record Failed!');window.location.href='../user/bill.php?id=".$user_id."'</script>";
			}
		}
	}
	else
