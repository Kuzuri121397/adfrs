			...
			if (isset($_POST['submit'])) 
			{
				...
				if(!empty($scheduled_event))
				{
            				...
            				if(in_array($fileActualExt,$allowed))
            				{
            					if($fileError === 0)
            					{
            						if($filesize < 1000000)
                					{
            							...
            							echo"<script>alert('Record Saved!'); window.location.href='../user/incoming.php?id=".$user_id."'</script>";
									}
									else
									{
					...
					else
					{
						...
						echo"<script>alert('Please Upload a file!'); window.location.href='../user/incoming.php?id=".$user_id."'</script>";
					}
