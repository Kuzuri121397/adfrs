            						...
            						if(in_array($fileActualExt,$allowed))
            						{
            							if($fileError === 0)
            							{
            								if($filesize < 1000000)
                							{
            									...
            									echo"<script>alert('Record Saved!'); window.location.href='../user/outgoing.php?id=".$user_id."'</script>";
											}
											else
											{
												echo"<script>alert('The size is too large! Upload File!'); window.location.href='../user/outgoing.php?id=".$user_id."'</script>";
											}
										}
										else
										{
											echo"<script>alert('There was an error in uploading the File!'); window.location.href='../user/outgoing.php?id=".$user_id."'</script>";
										}
									}
									else
            						{
              							echo"<script>alert('Invalid File Type!'); window.location.href='../user/outgoing.php?id=".$user_id."'</script>";
            						}
						
					}
					else
					{
						echo"<script>alert('Please upload a file!'); window.location.href='../user/outgoing.php?id=".$user_id."'</script>";
					}
			}
		}
	}
