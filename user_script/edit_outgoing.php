		...
		if(isset($_POST['submit'])) 
		{
			...
			$outgoing_remarks=$_POST['outgoing_remarks'];
					...
					if (!empty($fileTmpName)) 
					{
							...
							if($numFile > 0)
							{
								echo"<script>alert('A file already exists!'); window.location.href='../user/outgoing.php?id=".$user_id."'</script>";
							}
							else
							{
								$folder="../system/files";
									...
									if ($fileError === 0) 
									{
										if($filesize < 1000000)
										{
            								...
            								while ($rowResult=mysqli_fetch_array($selectResult)) 
            								{
            									...
            									echo"<script>alert('Updated Successfully!'); window.location.href='../user/outgoing.php?id=".$user_id."'</script>";
            								}
