		...
		while($row1=mysqli_fetch_array($result1)) 
		{
			...
			if (isset($_POST['submit'])) 
			{
				...
				$file=$_FILES['file'];
            				...
            				if(in_array($fileActualExt,$allowed))
            				{
            					if($fileError === 0)
            					{
            						if($filesize < 1000000)
                					{
            							...
            							echo"<script>alert('Record Saved!'); window.location.href='../user/voucher.php?id=".$user_id."'</script>";
									}
									else
									{
					...
					{
						echo"<script>alert('Please Upload a file!'); window.location.href='../user/voucher.php?id=".$user_id."'</script>";
					}#end of upload file
			}
			else
			{
